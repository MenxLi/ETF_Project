...
async function openTradeModal(sig) {
  ...
  if (rt?.price) {
    ...
    tradePriceSource.value = rt.source
  }
}
...
async function loadSellSignals() {
  ...
  errSell.value = ''
  try {
    sellSignals.value = await api('GET', '/api/sell-signals')
  } catch (e) {
    errSell.value = e.message
  } finally {
...
async function loadAlerts() {
  ...
  try {
    sellAlerts.value = await api('GET', '/api/alerts')
  } catch {}
...
async function runSellGenerator() {
  ...
  try {
    ...
    store.status = '生成失败: ' + e.message
  } finally {
    runningSellGen.value = false
  }
}
